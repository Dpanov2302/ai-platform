...
const TextToImagePage = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        setTimeout(() => {
            setImgUrl("https://placehold.co/512x320/png?text=Сгенерированное+изображение");
            setIsLoading(false);
        }, 1200);
    };
            ...
            <main className="max-w-4xl mx-auto px-4 py-24">
                <div
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        >
                            {isLoading ? (<><Loader
                                className="mr-2 h-4 w-4 animate-spin"/>Генерация...</>) : "Создать изображение"}
                        </button>
                    </form>
                    {imgUrl && (
                        <div className="mt-8">
                            <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Сгенерированное
                                изображение:</h2>
                            <div className="flex justify-center bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                <img src={imgUrl} alt="Сгенерированное изображение"
                                     className="rounded-md max-h-80 max-w-full shadow"/>
                            </div>
                        </div>
                    )}
                </div>
